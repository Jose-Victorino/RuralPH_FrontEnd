...
const newsService = createCRUD(TABLE_NAME)

const NewsModal = ({ modalState, setModalState, selectedRecord, handleModalSubmit }) => {
...
function AdminNews() {
  ...
  document.title = `News | Admin | Rural Rising PH`
  ...
  const handleModalSubmit = async (values, { setSubmitting }) => {
    ...
    const isInsert = modalState === 'INSERT'
  ...
  const handleDelete = async (id) => await newsService.deleteData(id, fetchData)
  ...
  return (
    <div className={s.container}>
      ...
      {modalState && <NewsModal {...{modalState, setModalState, selectedRecord, handleModalSubmit}}/>}
    </div>
  )
}
