...
function Input(props) {
  const { error, touched, id, displayName, type, required = false, ...rest } = props
  ...
  return (
    <div className={cn('flex-col', s.inputCont)}>
      {displayName &&
        <label htmlFor={inputId}>{wordCap(displayName)} {required && <span className={s.inputRequired}>*</span>}</label>
      }
      {type === 'textarea'
